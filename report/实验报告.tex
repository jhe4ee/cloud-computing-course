...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：docker compose 启动 + curl 返回 + SWR镜像列表 三张截图]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：kubectl get nodes -o wide 截图（3节点Ready，含VERSION列）]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：kubectl get pods（全Running） + 浏览器访问 ELB公网IP/api/ping 返回 \{"status":"ok"\} 截图]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入三张截图：PVC Bound + SET/GET 写入 + Pod删除后 GET 仍返回 hello]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：kubectl exec 验证 nginx.conf 包含 listen 8080 截图]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：Spark wordcount Driver Pod Completed 截图 + Top 10 words 日志截图]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：数据清洗日志截图（Schema + Missing values + 清洗后行数）]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：性能对比日志截图（local[1] 和 local[2] 的时间）]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：kubectl get pods -n monitoring（全Running）+ Grafana 健康检查返回 ok 截图]
  }%
}
...
\fbox{%
  \parbox{\textwidth}{%
    \centering
    [在此插入：GitHub Actions 流水线全部 Passed 绿色截图]
  }%
}
