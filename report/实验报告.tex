...
\midrule
PySpark local[1]（模拟单机 Pandas） & 1 & 7.46s \\
PySpark local[2]（双核并行）      & 2 & 2.03s \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Pandas vs PySpark 对比分析：}
PySpark local[1] 模式等价于 Pandas 单机处理——两者均使用单核 CPU 和单进程内存。当数据量达到 GB 级（远超本实验的 39MB），Pandas 会因单机内存耗尽而 OOM，而 PySpark 支持通过增加 executor 节点水平扩展至 TB 级数据。从表中可见，local[2] 双核耗时仅为 local[1] 的 $1/3.67$，证明了并行化带来的大幅加速。该对比说明：小数据用 Pandas 简单高效，大数据必须 PySpark。
